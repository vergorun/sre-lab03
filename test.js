...
export const options = {
  ...
  thresholds: {
    ...
    'http_req_duration{scenario:cities}': [{'threshold':'p(99)<400', abortOnFail: true, delayAbortEval: '10s'}], // 95% запросов должны укладываться в 300ms
    ...
    'http_req_duration{scenario:forecast}': [{'threshold':'p(99)<400', abortOnFail: true, delayAbortEval: '10s'}], // 95% запросов должны укладываться в 300ms
    'http_req_failed{scenario:weather_forecast}': [{'threshold': 'rate<0.01', abortOnFail: true, delayAbortEval: '10s'}], // доля http errors должна быть ниже 1%
    'http_req_duration{scenario:weather_forecast}': [{'threshold':'p(99)<5000', abortOnFail: true, delayAbortEval: '10s'}], // 95% запросов должны укладываться в 5000ms
  },
  scenarios: {
/*
    forecast: {
