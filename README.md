...
3. Через API внешним приложением(пользователем) читаются: список городов (GET /Citites) для резолва id в навания, отдельные прогнозы (GET /Forecast/{id}) и сводный прогноз (GET /WeatherForecast/), предположим, что соотношение пользователей, соверщающих запросы /городов/прогнозов отдельный/сводный составляет 10:10:1
...
Узким местом является реализация работы с базой данных, на один запрос API генерируется на порядок больше транзакций в DB, что приводит к тому, что при увеличении интенсивности параллельных запросов экспоненциально увеличивается время ответа, что не позволяет уложиться в требуемое время ответа (и процент ответов без ошибок) даже на средне заполненной базе, при увеличении количества записей в DB ситуация только ухудшается. Требуется переработка реализации работы с DB. 
Близкий к целевому показатель по времени ответа достигается только на практически пустой базе (1 сценарий - 10 городов/7 прогнозов на город), суммарный порог при этом около 35rps, целевые показатели по rps на заполненной базе не достигаются. 
При этом capacity тестирование можно признать успешным: целевое количество записей в справочниках поддерживается, данные успешно заносятся и извлекаются.
